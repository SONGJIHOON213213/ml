...
import warnings
from sklearn.datasets import load_fetch_covtype, load_boston, load_breast_cancer, load_digits,fetch_covtype
...
for i, model in enumerate(models):
    ...
    print(f"Model {i+1} - 기존 ACC: {acc:.5f}")
...
for i, model in enumerate(models):
    ...
    mse = mean_squared_error(y_test, y_predict)
    acc = accuracy_score(y_test, y_predict)
    print(f"Model {i+1} - 컬럼 삭제후 ACC: {acc:.5f}")
